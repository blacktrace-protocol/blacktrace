      ...
      for (const order of sortedOrders) {
        try {
          ...
          if (response.proposals && response.proposals.length > 0) {
            ...
            // Show proposals for Solana settlement (settlement_chain === 'solana' or undefined defaults to Solana)
            const settlementProposals = response.proposals
              .filter(p =>
                p.id &&
                p.id.trim() !== '' &&
                p.status === 'accepted' &&
                (p.settlement_chain === 'solana' || !p.settlement_chain) &&
            ...
            return (
              <div key={order.id} className="space-y-3">
                ...
                {proposals.map((proposal, idx) => {
                  ...
                  // Calculate amounts from zatoshi/lamports
                  const zecAmount = proposal.amount / 100000000; // zatoshi to ZEC
                  const priceSOL = proposal.price / 1000000000;  // lamports to SOL (price per ZEC)
                  const solAmount = zecAmount * priceSOL; // Total SOL
