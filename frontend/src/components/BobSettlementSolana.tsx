...
import { LAMPORTS_PER_SOL, PublicKey, Transaction, sendAndConfirmTransaction } from '@solana/web3.js';
...
// This would be fetched from the proposal in production
const ALICE_SOLANA_ADDRESS = 'F5B9wKjCMZ7y1EqKfLAeBqJsXfhLvZr8VNT9QHVnfrNY';
...
export function BobSettlementSolana({ onCountChange }: BobSettlementSolanaProps = {}) {
  ...
  const handleLockSOL = async (proposal: Proposal, amount: number, price: number) => {
    ...
    try {
      ...
      logSettlement('Locking SOL (demo mode)', 'alice_locked', {
        amount: `${totalSOL.toFixed(4)} SOL`,
        proposalId: proposal.id.substring(0, 8) + '...'
      });

      // DEMO SIMULATION: Direct transfer instead of actual HTLC lock
      // In production, this would call the HTLC program's lock instruction
      if (!solanaKeypair) {
        throw new Error('Solana wallet not connected');
      }

      const transaction = new Transaction();
      const alicePubkey = new PublicKey(ALICE_SOLANA_ADDRESS);

      // Transfer SOL directly to Alice (simulating HTLC lock)
      ...
      const signature = await sendAndConfirmTransaction(connection, transaction, [solanaKeypair]);
                      ...
                      {isAliceLocked && (
                        <>
                          ...
                          <Button
                            ...
                            onClick={() => handleLockSOL(proposal, proposal.amount / 100, proposal.price)}
