...
export const HashUtils = {
  ...
  /**
   * Compute SHA256 hash (compatible with Zcash and Solana)
   */
   ...
   * This is the cross-chain compatible hash function
   */
  computeHashLock: async (secret: string): Promise<string> => {
    const encoder = new TextEncoder();
    const secretBytes = encoder.encode(secret);
    const hashBytes = await HashUtils.sha256(secretBytes);
